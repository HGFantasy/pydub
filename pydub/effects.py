...
def normalize(seg, headroom=0.1):
    """
    headroom is how close to the maximum volume to boost the signal up to
    (specified in dB)
    """
...
def speedup(seg, playback_speed=1.5, chunk_size=150, crossfade=25):
    ...
    if len(chunks) < 2:
        raise Exception(
            f"Could not speed up AudioSegment, it was too short "
            f"{seg.duration_seconds:0.2f}s for the current settings:\n"
            f"{chunk_size}ms chunks at {playback_speed:0.1f}x speedup")
...
def invert_phase(seg, channels=(1, 1)):
    ...
    if channels == (1, 1):
        ...
        if seg.channels == 2:
            ...
            raise Exception(
                f"Can't implicitly convert an AudioSegment with "
...
def pan(seg, pan_amount):
    ...
    if not -1.0 <= pan_amount <= 1.0:
        raise ValueError(
            "pan_amount should be between -1.0 (100% left) "
            "and +1.0 (100% right)")
