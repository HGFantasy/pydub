...
audio_segment.low_pass_filter() instead of the slower, less powerful versions
    ...
    if gain_dB >= 0:
        if mode == "peak":
            sec = band_pass_filter(
                seg, focus_freq - bandwidth / 2,
                focus_freq + bandwidth / 2, order=order)
...
@register_pydub_effect
def eq(seg, focus_freq, bandwidth=100, channel_mode="L+R",
       filter_mode="peak", gain_dB=0, order=2):
    """
    Args:
        focus_freq - middle frequency or known frequency of band (in Hz)
        bandwidth - range of the equalizer band
    ...
    if channel_mode == "L":
        ...
        seg = [_eq(seg[0], focus_freq, bandwidth, filter_mode, gain_dB,
                   order), seg[1]]
        return AudioSegment.from_mono_audiosegments(seg[0], seg[1])

    if channel_mode == "R":
        seg = seg.split_to_mono()
        seg = [seg[0], _eq(seg[1], focus_freq, bandwidth, filter_mode,
                           gain_dB, order)]
        return AudioSegment.from_mono_audiosegments(seg[0], seg[1])
